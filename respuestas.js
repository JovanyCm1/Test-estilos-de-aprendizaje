...
function resultado() {
    ...
    function asignarPuntos(
        ...
        for (var i = 0; i < radios.length; i++) {
            if (radios[i].checked) {
                var sectionElement = radios[i].closest('section');
                var preguntaTexto = sectionElement ? sectionElement.querySelector('strong').textContent.trim() : "Pregunta no encontrada";

                var respuestaTexto = radios[i].parentNode.textContent.trim();

                preguntas.push(preguntaTexto);
                respuestas.push(respuestaTexto);
    ...
    // Mostrar las respuestas, preguntas y conteos en la consola
    ...
    resultadosAprendizaje.formulario = preguntasRespuestasString;

    //Se guardan los datos y son accesibles desde cualquier lado
    localStorage.setItem(
        "resultadosAprendizaje",
        JSON.stringify(resultadosAprendizaje)
    );
